...
def play_sound(stype):
    ...
    try:
        ...
        if stype == "success":
            ...
            if os.path.exists(p):
                # SND_ASYNC giúp âm thanh phát ngầm, KHÔNG làm đơ phần mềm
                winsound.PlaySound(p, winsound.SND_FILENAME | winsound.SND_ASYNC)
            else:
                winsound.Beep(1000, 150)
        elif stype == "error":
            p = os.path.join(base_dir, ERROR_SOUND_FILE)
            if os.path.exists(p):
                winsound.PlaySound(p, winsound.SND_FILENAME | winsound.SND_ASYNC)
            else:
    ...
    def run(self):
        while self.running:
            try:
                ...
                ws.update(values=[[sid, p_type, task]], range_name=f'C{row_idx}:E{row_idx}', value_input_option='USER_ENTERED')
...
# --- LUỒNG KHỞI TẠO HỆ THỐNG BAN ĐẦU ---
class InitSystemWorker(QThread):
    ...
    def run(self):
        blacklist, users_data, user_cookies = {}, {}, {}
        try:
...
class ChangePasswordDialog(QDialog):
    def __init__(self, parent=None, current_uid="", is_admin=False, users_data=None):
        ...
        self.current_uid, self.is_admin, self.users_data = current_uid, is_admin, users_data or {}
        ...
        self.setStyleSheet("""
            ...
            QLineEdit, QComboBox { padding: 12px; border-radius: 8px; font-size: 14px; background: white; border: 1px solid #CBD5E1; color: #0F172A; }
            QLineEdit:focus, QComboBox:focus { border: 2px solid #3B82F6; }
            QLineEdit:disabled { background: #E2E8F0; color: #64748B; }
            QPushButton { font-family: 'Segoe UI'; font-size: 15px; font-weight: bold; border-radius: 8px; min-height: 45px; color: white; background-color: #3B82F6; border: none; margin-top: 15px; }
        ...
        if self.is_admin:
            ...
            self.target_input = QLineEdit(f"{self.current_uid} - {self.users_data.get(self.current_uid, {}).get('name', '')}")
    ...
    def process_update(self):
        ...
        op, np, cp = self.old_pass.text().strip(), self.new_pass.text().strip(), self.confirm_pass.text().strip()
...
class ShopeeScannerTactile(QMainWindow):
    def __init__(self):
        ...
        # BỘ ĐỆM: Lưu trữ các mã đang đợi API trả lời (Chống quét liên thanh bị lag/trùng)
        self.processing_sids = set()

        self.is_fully_loaded = False
        self.blacklist, self.users_data, self.all_user_cookies = {}, {}, {}
        self.current_uid, self.current_is_admin, self.active_cookie, self.active_csrf = "", False, "", ""
    ...
    def init_ui(self):
        ...
        self.setStyleSheet("""
            QMainWindow, QStackedWidget { background-color: #F1F5F9; }
        ...
        self.user_info_lbl.setStyleSheet("color: #0F172A; font-size: 15px; font-weight: 900; background: white; padding: 6px 15px; border-radius: 10px; border: 1px solid #CBD5E1;")
        ...
        self.btn_change_pass.setStyleSheet("color: #3B82F6; font-size: 12px; font-weight: bold; background: transparent; border: none; text-align: right; text-decoration: underline;")
    ...
    # --- ĐÃ XÓA KHÓA UI (SETENABLED) ĐỂ CHỐNG KẸT KHI QUÉT NHANH ---
    def handle_scan(self):
        sid = self.input_box.text().strip().upper()
        self.input_box.clear()
        
        # Luôn focus lại ô text để sẵn sàng nhận mã tiếp theo ngay lập tức
        self.input_box.setFocus()
        
        if not sid: return

        # 1. Kiểm tra xem mã có đang trong hàng đợi xử lý mạng không?
        if sid in self.processing_sids:
            self.log(f"⏳ BỎ QUA: {sid} đang được hệ thống xử lý...", "#F59E0B")
            return

        # 2. Kiểm tra trùng lặp mã đã thành công
        ...
        if sid in self.blacklist:
            ...
            if bl_task == "" or bl_task == "OUTBOUND":
                is_blocked = True
                block_reason = "Tất cả (Outbound)" if bl_task == "OUTBOUND" else "Tất cả"
            elif bl_task in ["DP", "WIS"] and current_task in ["DP", "WIS"]:
                is_blocked = True
                block_reason = f"Nhóm DP/Wis (Gốc: {bl_task})"
        ...
        # 4. Đưa vào danh sách đang xử lý & Đẩy vào hàng đợi (Không khóa Form)
    ...
    def on_scan_done(self, sid, name, success):
        # Mạng trả kết quả về -> Xóa khỏi danh sách đang xử lý
        if sid in self.processing_sids:
        ...
        if success:
            ...
            self.status_lbl.setText(f"● TỔNG ĐÃ QUÉT: {len(self.scanned_list)}")
        else:
    ...
    def log(self, msg, color=None):
        ...
        self.log_view.append(f"<span style='color: #64748B;'>[{t}]</span> <span style='color: {final_color};'><b>{msg}</b></span>")
